...
for (r in startRaster:length(rasters)){
		...
		if (all(extent(valueCropped)==extent(zoneCropped), res(valueCropped)==res(zoneCropped), projection(valueCropped)==projection(zoneCropped))){
			...
			naCol=paste(zoneType, "_", strsplit(x=rasters[r], "\\.")[[1]][1], "_NA", sep="")
			colnames(tabDataNA)[ncol(tabDataNA)]=naCol
			CATCHID=as.numeric(rownames(tabDataNA))
			variableData=cbind(CATCHID, tabDataNA)
			finalData=merge(finalData, variableData, by.x ="CATCHID", by.y ="CATCHID" , all.x = TRUE,sort=FALSE)
			finalData[,which(colnames(finalData)==naCol)]=finalData$cellCount-finalData[,which(colnames(finalData)==naCol)]
