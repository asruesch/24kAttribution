...
if __name__ == "__main__":
    try:
        ...
        if tb: # Check to see if Relationships table exists in the workspace. If it does, then run the rest of the code
                            ...
                            if exists > 0:
                                ...
                                for value1 in edgeList: # This for loop searches for a link in another list
                                    ...
                                    if ind2 > 0: # if value1 is in edgeList2 then you know that can find their link in FindDistance Function
                                        ind2 = edgeList2.index(value1) # This finds the index value of value1 in list
                                        if ind2 > 0: # if the index value is not at first element
                                            if count == 0:
                                                distance = FindDistance(edgeList2, EdgeSegDistList[Index2], value1, 1)
                                                distance = abs(distance - EdgeSegDistList[Index1][0]) # this subtracts the down stream distance from the total length
                                            else:
                                                distance = FindDistance(edgeList, EdgeSegDistList[Index1], value1, 0)
                                                distance = abs(distance + FindDistance(edgeList2, EdgeSegDistList[Index2], value1, 0))
                                        else:
                                            distance = FindDistance(edgeList, EdgeSegDistList[Index1], value1, 1)
                                            distance = abs(distance - EdgeSegDistList[Index2][0]) # this subtracts the down stream distance from the total length
                                        valuefound = 1
            ...
            gp.AddMessage(" ")
            gp.AddWarning("FINISHED Instream Distance (Symmetric) Script")
            gp.AddMessage(" ")
            gp.AddMessage(" ")
            gp.AddMessage(" ")
