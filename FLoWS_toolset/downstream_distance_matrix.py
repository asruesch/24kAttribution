...
if __name__ == "__main__":
    try:
        ...
        if tb: # Check to see if Relationships table exists in the workspace. If it does, then run the rest of the code
            ...
            # this loop goes through each edge list associated with a sample edge and finds downstream edges from it.
            #gp.AddMessage(str(SourceEdgeList))
            #gp.AddMessage("  ")
            #gp.AddMessage(str(EdgeSegDistList))
            #gp.AddMessage("  ")
            #gp.AddMessage(str(EdgeSegList))
            ...
            for edgeList in EdgeSegList:
                ...
                for SourceEdge in SourceEdgeList:
                    ...
                    if index <> Index1:
                        ...
                        if exists > 0:
                            #ind = SourceEdgeList[Index1:].index(SourceEdge) # this is to get the index of the edge so the correction value can be subtracted from the total length
                            ind = matrixIDList.index(matrixIDList[index])
                            distance = FindDistance(edgeList, EdgeSegDistList[Index1], SourceEdge, 1)
                            distance = distance - EdgeSegDistList[ind][0]
                            if distance > 0:
